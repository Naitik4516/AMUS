...
>
    <div
        class="flex flex-col gap-3 p-6 rounded-full bg-zinc-900/20 border-zinc-700/30 border-2 text-white shadow-lg my-auto -translate-y-20 z-40 w-21"
    >
        {#each menuItems as item (item)}
            <button
                animate:flip
                onclick={() => goto(item.route)}
                class={`group flex items-center w-11 h-11 -ml-1 hover:w-32 focus:w-48
